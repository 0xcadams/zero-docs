...
| `initial_sync_runs`                  | Counter   |       | Number of initial-sync runs. Has `sync_mode`, `copy_format`, and `result` attributes                                                            |
| `initial_sync_duration`              | Histogram | s     | Wall-clock duration of an initial-sync run. Has `sync_mode`, `copy_format`, and `result` attributes                                             |
| `initial_sync_copy_duration`         | Histogram | s     | Wall-clock duration of the COPY phase for a successful initial-sync run. Has `sync_mode`, `copy_format`, and `result` attributes                |
| `initial_sync_copy_other_duration`   | Histogram | s     | Initial-sync duration excluding SQLite flush and index time for a successful run. Has `sync_mode`, `copy_format`, and `result` attributes       |
| `initial_sync_flush_duration`        | Histogram | s     | Total SQLite flush time for a successful initial-sync run. Has `sync_mode`, `copy_format`, and `result` attributes                              |
| `initial_sync_index_duration`        | Histogram | s     | SQLite index creation time for a successful initial-sync run. Has `sync_mode`, `copy_format`, and `result` attributes                           |
| `initial_sync_rows`                  | Counter   |       | Rows copied during successful initial-sync runs. Has `sync_mode`, `copy_format`, and `result` attributes                                        |
| `initial_sync_copy_stream`           | Counter   | bytes | PostgreSQL COPY stream bytes processed during initial sync, including in-progress and failed runs. Has `sync_mode` and `copy_format` attributes |
| `initial_sync_completed_copy_stream` | Counter   | bytes | PostgreSQL COPY stream bytes processed during successful initial-sync runs. Has `sync_mode`, `copy_format`, and `result` attributes             |
| `initial_sync_copy_chunks`           | Counter   |       | PostgreSQL COPY stream chunks processed during initial sync. Has `sync_mode` and `copy_format` attributes                                       |
...
| `serving-lag`                       | Gauge         | ms   | Maximum time active ViewSyncer client groups have had replica changes ready locally but not yet served to clients                                 |
| `serving-lag-stats`                 | Gauge         | ms   | Distribution of serving lag across active ViewSyncer client groups. Has a `stat` attribute: `min`, `p50`, `p75`, `p99`, `max`                     |
| `serving-lagging-client-groups`     | Gauge         |      | Number of active ViewSyncer client groups with replica changes ready locally but not yet served to clients                                        |
