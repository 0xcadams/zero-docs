...
<Note type="note" heading="Native histogram compatibility">
  The `zero.sync.view_syncer_lag` and
  `zero.sync.view_syncer_hydration` metrics use native
  histograms. Prometheus ingestion for these metrics
  requires Prometheus `3.8+`.
</Note>
...
| `api.requests`            | Counter       |      | API requests to user mutate, query, cleanup, and auth endpoints |
| `api.request_duration`    | Histogram     | s    | End-to-end API request duration, including retries              |
| `api.attempts`            | Counter       |      | HTTP fetch attempts made while calling user API endpoints       |
| `api.attempt_duration`    | Histogram     | s    | API HTTP fetch attempt duration, excluding retry sleep          |
| `api.in_flight`           | UpDownCounter |      | API requests currently in flight                                |
| `startup_duration`        | Histogram     | s    | Duration from starting zero-cache to its ready signal           |
| `worker_startup_duration` | Histogram     | s    | Duration from starting a worker to its ready signal             |
...
| `litestream.backup.process_duration`       | Histogram | s     | Runtime duration of Litestream backup subprocesses before they exit                                                     |
| `litestream.backup.list_duration`          | Histogram | s     | Duration of Litestream backup destination listing commands                                                              |
| `litestream.backup.verification_duration`  | Histogram | s     | Duration of verifying actual backup state in the backup destination                                                     |
| `litestream.snapshot.reservation_duration` | Histogram | s     | Duration of snapshot reservations while view-syncers restore and subscribe                                              |
...
| `slot_health`                        | Gauge     | 1     | Health of the active logical replication slot. Uses `slot` and `status` attributes                                  |
| `slot_retained_wal_bytes`            | Gauge     | bytes | WAL bytes retained by the active logical replication slot                                                           |
| `slot_safe_wal_bytes`                | Gauge     | bytes | WAL bytes the active logical replication slot can retain before loss                                                |
...
| `shadow-sync-runs`                   | Counter   |       | Number of [shadow initial-sync](/docs/zero-cache-config#shadow-sync-enabled) runs                                   |
| `shadow-sync-duration`               | Histogram | s     | Wall-clock duration of a shadow initial-sync run                                                                    |
...
| `serving_lag`                       | Gauge         | ms   | Maximum time active ViewSyncer client groups have had replica changes ready locally but not yet served to clients    |
| `serving_lag_stats`                 | Gauge         | ms   | Distribution of serving lag across active ViewSyncer client groups                                                   |
| `serving_lagging_client_groups`     | Gauge         |      | Number of active ViewSyncer client groups with replica changes ready locally but not yet served to clients           |
| `view_syncer_lag`                   | Histogram     | s    | Lag from replica-ready changes to ViewSyncer output for active client groups                                         |
| `view_syncer_hydration`             | Histogram     | s    | Time from ViewSyncer query sync requiring hydration to output for a client group                                     |
| `lock-wait-time`                    | Histogram     | s    | Time spent waiting to acquire the ViewSyncerService lock per operation                                               |
| `pipeline-resets`                   | Counter       |      | Count of pipeline resets                                                                                             |
...
| `query.row-set-signature-drifts`    | Counter       |      | Times unchanged query rehydration produced a different row-set signature than the CVR, forcing a config-version bump |
