...
Zero publishes the same container image to Docker Hub and GHCR.

- Docker Hub: `rocicorp/zero:{version}`
- GHCR: `ghcr.io/rocicorp/zero:{version}`

The examples below use Docker Hub. If you prefer GHCR, replace the image name with `ghcr.io/rocicorp/zero:{version}`.
