...
Comma-separated list of client-provided custom headers that zero-cache is allowed to forward to your mutate endpoint. Header names are matched case-insensitively.
...
Comma-separated list of incoming HTTP request headers that zero-cache is allowed to forward to your mutate endpoint. Use this for headers injected by a proxy or load balancer, such as `x-forwarded-for` or `cf-ray`.

Unlike [mutate allowed client headers](#mutate-allowed-client-headers), these headers are read from the HTTP request that established the connection, not from client-provided custom headers. Header names are matched case-insensitively.

By default, no request headers are forwarded.
...
Comma-separated list of client-provided custom headers that zero-cache is allowed to forward to your query endpoint. Header names are matched case-insensitively.
...
Comma-separated list of incoming HTTP request headers that zero-cache is allowed to forward to your query endpoint. Use this for headers injected by a proxy or load balancer, such as `x-forwarded-for` or `cf-ray`.

Unlike [query allowed client headers](#query-allowed-client-headers), these headers are read from the HTTP request that established the connection, not from client-provided custom headers. Header names are matched case-insensitively.
