...
See [`canary-runbook.md`](canary-runbook.md) for the shadow-only rollout,
queries, acceptance gates, and rollback procedure.
...
- `scripts/run-linux.mjs`: constrained ARM64 Linux runner for the mmap cap
  sweep; dry-run unless `--execute`.
...
| `linux-mmap-sweep` | 256-1024 MiB mmap cap sweep in constrained ARM64 Linux       |
| `linux-mmap-knee`  | Confirm candidate caps at 400 MB and 1.2 GB in Linux         |
...
## Linux mmap Cap Sweep

The Linux runner matches the tighter large-stack production shape observed in
EU Central: ARM64 Linux, 3 GiB container memory, no swap, 1 CPU, and a 2304 MiB
Node heap cap. PostgreSQL runs in a separate container so its memory is not
charged to the measured benchmark cgroup. Each run records cgroup peak memory,
OOM counters, process RSS, the requested mmap cap, and SQLite's effective mmap
size.

Inspect the balanced 20-process matrix without starting Docker resources, then
execute it explicitly:

```sh
node .releases/1.8/benchmarks/initial-sync-bottleneck/scripts/run-linux.mjs linux-mmap-sweep
node .releases/1.8/benchmarks/initial-sync-bottleneck/scripts/run-linux.mjs linux-mmap-sweep --execute
node .releases/1.8/benchmarks/initial-sync-bottleneck/scripts/aggregate.mjs linux-mmap-sweep
```

The completed evidence-directed matrix contains 309 successful processes,
including 52 constrained ARM64 Linux cap-selection runs. Stage 3 worker scaling
and the original confirmation matrix were superseded by the index mechanism
controls and paired default/mmap cross-version matrix.
