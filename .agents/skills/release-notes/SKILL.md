...
- **Performance descriptions must explain the developer impact**, not just repeat a commit title, internal optimization, or benchmark result:
  ...
  - For replication throughput charts with variable-size rows, prefer generated payload throughput in `MiB/s` over `changes/s`. In benchmark artifacts, state whether the byte size is fixture payload, WAL bytes, logical-replication bytes, or wire bytes; do not imply unmeasured byte semantics in public prose.
